...
const globalEnv = environment => {
  ...
  if (environment !== 'production' || env === undefined) return undefined
...
const server = (base, main, nodeServer) => {
  const entry = nodeServer === undefined ? resolve(main) : resolve(nodeServer)
...
const retrieveServerConfig = opts => {
  ...
  const { main, nodeServer } = opts
  return server(baseConfig, main, nodeServer)
}
